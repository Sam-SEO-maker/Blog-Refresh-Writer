...
1. **`recherche-sources <sujet\|url>`** — **recherche documentaire** : collecte de sources autoritatives fraîches (études, stats 2024-2026, experts) pour nourrir le brief E-E-A-T. Wrappe la skill `deep-research` + `WebSearch`/`WebFetch`. **À construire** (le pipeline n'a aucune recherche de sources aujourd'hui — cf. étape 5bis). Invocable seule (« documente-moi ce sujet ») **et** appelée par l'orchestrateur avant la génération.

"""Catalogue Superprof + onboarding site (Phase 6d) — sans réseau.
...
from scripts.utils import site_onboard as T
...
def test_catalog_keeps_only_blog_and_known_ressources():
    ...
    res_ids = {r["site_slug"] for r in cat["sites"] if r["type"] == "ressources"}
...
def test_suisse_deux_blogs_non_dedupliques():
    ...
    site_slug aux deux et la déduplication en perdait un (régression 2026-07-17).
    """
...
def fake_root(monkeypatch):
    with tempfile.TemporaryDirectory() as td:
        ...
        (root / "_shared" / "config" / "superprof_sites_catalog.json").write_text(
        ...
        monkeypatch.setattr(T, "CATALOG_PATH",
                            root / "_shared" / "config" / "superprof_sites_catalog.json")
...
def test_onboard_rejects_unknown_id(fake_root):
    with pytest.raises(ValueError):
        T.onboard_site("xx-zz-blog", base_path=fake_root, no_sparse=True)
